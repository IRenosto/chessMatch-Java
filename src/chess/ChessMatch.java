...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (target.getColumn() == source.getColumn() - 1 && capturedPiece == null) {
				capturedPiece = board.removePiece(new Position(source.getRow(), source.getColumn() - 1));
			}
			if (target.getColumn() == source.getColumn() + 1 && capturedPiece == null) {
				capturedPiece = board.removePiece(new Position(source.getRow(), source.getColumn() + 1));
			}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof Pawn) {	
			...
			if (target.getColumn() != source.getColumn() && capturedPiece == enPassantVulnerable) {
				board.placePiece(pawn, ((p.getColor()==Color.WHITE) ? new Position(4, target.getColumn()) : new Position(3, target.getColumn()) ));
			}
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> currentColor = piecesOnTheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
				.collect(Collectors.toList());

		for (Piece p : currentColor) {
			boolean[][] mat = p.possibleMoves();
			for (int i = 0; i < board.getRows(); i++) {
